...
def main():
    """Main function to convert all CSV files in the data_processing folder."""
    data_processing_folder = Path("data_processing")
    data_folder = Path("data")
    
    if not data_processing_folder.exists():
        print(f"Data processing folder not found: {data_processing_folder}")
        return
    ...
    # Find all CSV files
    csv_files = list(data_processing_folder.glob("*.csv"))
    
    if not csv_files:
        print("No CSV files found in data_processing folder")
        return
    ...
    for csv_file in csv_files:
        # Create output JSONL filename in data folder
